...
<html>
	...
	<form action="grader.php" method="post" id="form">
		<div id="name"><h2>문제<input name="no" value="<?php $url = basename($_SERVER['PHP_SELF']); $tok = explode('.', $url);	echo trim($tok[0]); ?>
					...
					?></h2></div>
		<aside>
			<p style="margin:2%;margin-right:6%;font-size:13px;line-height:1.5em;"><strong style="font-size:16px;">[문 제]</strong><br/><br/>하나, 두리, 샛별은 유명한 춤꾼이다. 평소 자신이 제일이라며 자존심 싸움을 하던 세명은 최고의 춤꾼을 가리기 위해 댄스 경연대회에 참가하였고, 마침내 결승에서 다시 만나게 되었다. 결승의 진행방식은 세명의 심사위원 A, B, C가 매긴 점수의 총 합으로 우승자를 가린다. 우리가 할 일은 심사위원들의 점수를 보고 우승자를 가려내는 것이다.<br/><br/>입력은 총 세 줄로 한줄씩 하나, 두리, 샛별이 심사위원 A, B, C에게 차례로 받은 점수가 주어진다. 점수는 1점부터 5점까지 존재하며 항상 우승자를 가릴 수 있게 주어진다.<br/><br/>출력은 두 줄로 첫 줄에는 각 참가자의 점수의 총 합을 차례로 출력하고, 두번째 줄에는 우승자의 이름을 출력한다.</p>
<br/><br/>
					<p style="margin:2%;font-size:16px;line-height:1.5em;"><strong>[예제 입력]<br/></strong>5 4 4<br/>5 5 4<br/>3 4 4<br/></p>
					<br/><br/>
					<p style="margin:2%;font-size:16px;line-height:1.5em;"><strong>[예제 출력]<br/></strong>13 14 11<br/>두리<br/></p>
					<br/><br/>
		</aside>
		<section>
				&nbsp;&nbsp;&nbsp;문제푸는 곳<br/>
